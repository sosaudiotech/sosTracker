...
def calibrate(objpoints, imgpoints, image_size, square_cm, pattern, samples_list, flipped):
    ...
    for i in range(len(objpoints)):
        ...
        n = len(imgpoints[i]); total_err += err * err; total_pts += n
...
def run_folder_mode(args):
    ...
    result = calibrate(objpoints, imgpoints, image_size, args.square_cm, (pat_cols, pat_rows), samples, args.flip)
    with open(args.save_json, "w") as f:
        json.dump(result, f, indent=2)
    print(f"[OK] Saved intrinsics → {args.save_json}")
    print(f"[INFO] RMS reprojection error: {result['rms_reproj_error']:.4f}")
    print(f"[INFO] Samples used: {taken}")
